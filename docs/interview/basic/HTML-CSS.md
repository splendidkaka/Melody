...
- 盒子模型的处理：在标准模式下，盒子模型的宽度`width`和高度`height`包括内容区域、内边距`padding`、边框`border`和外边距`margin`；而在怪异模式下，盒子模型的宽度和高度则只包括内容区域和边框。这种差异可能导致布局上的不一致。
...
/>
<meta name="description" content="melodykira" />
...
```html
<meta name="author" content="melodykira" />
```
...
```
- 作用：强制浏览器不缓存页面（适用于需要实时更新的页面）

:::

## DOM、HTML 与 window 对象的区别与联系？

参考答案

::: details
`HTML文件 → 浏览器解析 → 创建DOM树 → 挂载到window对象`
           `(解析器)      (内存模型)    (全局访问点)`
...
| 概念        | 定义                                                                 | 特点                                                                 |
|-------------|----------------------------------------------------------------------|----------------------------------------------------------------------|
| **HTML**    | 超文本标记语言(HyperText Markup Language)                           | 静态的文本文件，描述网页的结构和内容                                 |
| **DOM**     | 文档对象模型(Document Object Model)                                  | HTML文档的内存表示，动态的、可编程的接口                             |
| **window**  | 浏览器窗口的JavaScript全局对象                                       | 代表浏览器窗口，是DOM的顶层对象，包含所有全局变量和函数              |
...
🌐 语义化结构标签
```html
<header>页面或区块页眉</header>
<nav>导航链接</nav>
<main>主要内容区</main>
<article>独立内容块</article>
<section>文档章节</section>
<aside>侧边栏内容</aside>
<footer>页面或区块页脚</footer>
```
🎵 多媒体标签
```html
<audio controls>
...
<time datetime="2023-05-20">5月20日</time>
```
:::
写一个 HTML5 语义化的例子
...
|---------------------|--------------------------------|-------------------------------|
| **存储位置**        | HTML 标签上                    | DOM 对象内存中                |
| **值类型**          | 始终是字符串                   | 可以是任意JS类型              |
| **同步行为**        | 修改不会自动更新property       | 修改可能影响attribute         |
| **大小写敏感**      | 不敏感 (`class` vs `CLASS`)    | 敏感 (`className`)            |
| **自定义扩展**      | 支持任意自定义属性             | 仅限DOM标准定义的属性         |

🔍 详细对比

1. 访问方式不同
```html
<input id="user" type="text" value="初始值" data-custom="自定义">
```
js操作
```js
...
// property 访问
console.log(input.value);  // 用户当前输入的值
input.value = '新特性值';
```
...
📌 核心性能原则
1. 减少回流(reflow)次数：DOM操作会触发页面重新计算布局
2. 减少DOM访问次数：JS与DOM交互存在性能开销
3. 合理利用现代API：优先使用浏览器优化后的方法
...
```
方案2 
- requestAnimationFrame分批插入
```js
// 大数据量时避免阻塞主线程
function batchInsert(total, chunkSize = 50) {
  let index = 0;
...
#### 📊 对比表格
| 属性                | 包含滚动条 | 包含padding | 包含border | 包含溢出内容 | 典型用途                     |
|---------------------|------------|-------------|------------|--------------|------------------------------|
| **offsetHeight**    | ✅         | ✅          | ✅         | ❌           | 获取元素布局占用总高度         |
| **clientHeight**    | ❌         | ✅          | ❌         | ❌           | 获取可视区域高度               |
| **scrollHeight**    | ❌         | ✅          | ❌         | ✅           | 检测内容是否溢出/需要滚动      |

#### 💻 代码示例
```html
<div id="example" style="height: 100px; padding: 20px; border: 5px solid; overflow: auto;">
...
console.log('offsetHeight:', el.offsetHeight);  // 150 (100 + 20x2 + 5x2)
console.log('clientHeight:', el.clientHeight);  // 140 (100 + 20x2)
console.log('scrollHeight:', el.scrollHeight);  // 340 (300 + 20x2)
</script>
```
#### 🌟 实用场景
1. 检测内容是否溢出
...
```
5. 获取元素相对于视口的高度（含滚动偏移）
```js
const rect = document.getElementById('myElement').getBoundingClientRect();
const height = rect.height; // 元素的总高度
const top = rect.top;       // 元素顶部相对于视口的距离
```

![rect](./source/rect.png)

#### ⚠️ 注意事项
- 隐藏元素：这些属性对display: none的元素返回0
- 小数像素：结果会被四舍五入为整数
- 性能影响：频繁读取会触发回流（reflow），避免在循环中使用
- content-box模式：clientHeight不包含border
- border-box模式：clientHeight包含border（此时与offsetHeight相同）
:::
