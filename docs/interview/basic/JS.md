...
| 类型       | 示例                  | 描述                          | typeof 返回值 |
|------------|-----------------------|-----------------------------|--------------|
| Number     | `42`, `3.14`, `NaN`   | 整数和浮点数                  | "number"     |
| String     | `"hello"`, `` `hi` `` | 文本数据                      | "string"     |
| Boolean    | `true`, `false`       | 逻辑值                        | "boolean"    |
| Null       | `null`                | 表示空值                      | "object"     |
| Undefined  | `undefined`           | 未定义的默认值                 | "undefined"  |
| Symbol     | `Symbol('id')`        | 唯一标识符 (ES6)              | "symbol"     |
| BigInt     | `123n`                | 大整数 (ES2020)               | "bigint"     |
...
42 instanceof Number   // false (基本类型不适用)
```
...
### 🎯 最佳实践
- 使用 === 判断 null
- 使用 typeof 或 === 判断 undefined
- 使用 Array.isArray() 判断数组
- 复杂类型使用 Object.prototype.toString.call()
- 在TypeScript中，可以使用typeof和instanceof作为类型守卫(type guard)
:::
...
Array.from('123') // ['1', '2', '3']   还可以用来把伪数组转为数组   set转为数组
Array.from(document.querySelectorAll('div')) // 类数组转换
[...'hello']           // ['h','e','l','l','o']
```
...
```js
let arr = [1, 2]
arr.push(3) // [1, 2, 3]  return 新的长度
arr.pop() // [1, 2]       return 被删除的函数
arr.unshift(4) // [4, 1, 2] return 新的长度
arr.shift() // [1, 2]       return 删除的元素
```
...
for (const key in arr) {
  console.log(key); // 输出: 0, 1, 'customProp'
}
```
解决方法：
```js
...
对数组遍历比for...in更快（不涉及原型链检查）
 🛠️ 如何选择遍历方法？

| 场景                      | 推荐语法                          | 示例代码                                                                 |
|---------------------------|----------------------------------|--------------------------------------------------------------------------|
| **遍历普通对象的属性**     | `for...in` + `hasOwnProperty`检查 | ```for (const key in obj) { if (obj.hasOwnProperty(key)) { ... } }```    |
| **遍历数组元素值**         | `for...of` 或 `forEach()`         | ```for (const value of arr) { ... }```<br>```arr.forEach(value => { ... })``` |
| **遍历Map/Set等集合**      | `for...of`                        | ```for (const [key, value] of map) { ... }```                            |
| **需要索引和值**           | `array.forEach()`                 | ```arr.forEach((value, index) => { ... })```                              |
| **需要提前终止遍历**        | `for...of` 或 `for`循环            | ```for (const val of arr) { if (condition) break; }```                    |
| **需要同时遍历键值对**      | `Object.entries()` + `for...of`   | ```for (const [key, value] of Object.entries(obj)) { ... }```             |

💡 选择依据

1. **安全性**：
   - 对象遍历必须用`hasOwnProperty`检查
   - `for...of`不会遍历原型链属性

2. **性能**：
   - 数组遍历：`for...of` > `forEach` > `for...in`
   - 大数据集避免使用`for...in`

3. **功能性**：
   - 需要索引时用`forEach`
   - 需要提前终止用`for...of`或传统`for`循环
...
- 并行执行：同时运行所有传入的 Promise，等待全部完成。
- 结果顺序：返回的结果数组顺序与输入的 Promise 顺序一致（即使完成时间不同）。
- 快速失败：如果任意一个 Promise 被拒绝（reject），Promise.all 会立即拒绝，并返回第一个错误。

- 场景：并行执行多个异步操作，等待所有完成
```js
// 同时请求多个接口数据
...
// 备用方案降级
const primaryData = await Promise.any([
  getPremiumData().catch(() => {}),
  getStandardData()
]);
```
